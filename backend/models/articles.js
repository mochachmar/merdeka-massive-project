...
const createArticle = async (article) => {
  ...
  const [result] = await db.query('INSERT INTO articles (title, thumbnail_image, short_description, long_description, publish_date, status, created_by) VALUES (?, ?, ?, ?, ?, ?, ?)', [
    title,
    thumbnail_image,
    short_description,
    long_description,
...
const updateArticle = async (id, article) => {
  const { title, thumbnail_image, short_description, long_description, publish_date, status, created_by } = article;
  const [result] = await db.query('UPDATE articles SET title = ?, thumbnail_image = ?, short_description = ?, long_description = ?, publish_date = ?, status = ?, created_by = ? WHERE article_id = ?', [
    title,
    thumbnail_image,
    short_description,
    long_description,
