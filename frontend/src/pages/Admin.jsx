...
export default function Admin() {
  ...
  const [jumlahArtikel, setJumlahArtikel] = useState(0); // State to store the number of articles from API
  ...
  useEffect(() => {
    const fetchPanduanCount = async () => {
      try {
        ...
        setJumlahArtikel(data.length); // Assuming the response returns an array of guides
      } catch (error) {
