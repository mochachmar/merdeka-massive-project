...
const IdentifikasiAI = () => {
  ...
  useEffect(() => {
    const detectPlant = async () => {
      ...
      detectCalled.current = true; // Mark that the process has been run
      setLoading(true);
  ...
  useEffect(() => {
    const uploadPlantData = async () => {
      ...
      // Mapping prediction result to health_status and disease_name
      ...
      formData.append(
        "scientific_name",
        plantsData[location.state.plantType]?.scientificName ||
          "Scientific name not found"
      );
      formData.append(
        "description",
        topPrediction.class_label || "No description"
      );
      formData.append(
        "care_instructions",
        topPrediction.solution || "No care instructions"
      );
      formData.append("health_status", health_status);
      formData.append("disease_name", disease_name);
      formData.append("created_by", "2"); // Replace with appropriate user_id
      ...
      <div className="flex-grow relative mt-4 mx-auto w-full max-w-4xl px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 px-4 mb-12">
          ...
          <div className="bg-transparent bg-opacity-50 border border-[#45543D] shadow-lg rounded-lg p-4">
            ...
            <ReactMarkdown>
              {predictions.length > 0 ? predictions[0].solution : t("loading")}
            </ReactMarkdown>
          </div>
        </div>
      </div>
