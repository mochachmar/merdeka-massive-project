...
function DeteksiPenyakit() {
  ...
  //       "http://localhost:3000/api/upload-tanaman",
  //       formData,
  //       {
  ...
  return (
    <div className="flex flex-col min-h-screen w-full">
      ...
      <div className="flex flex-col items-center flex-grow p-6 md:p-14 bg-gray-50">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-8">Deteksi Penyakit Tanaman Anda</h1>

        <div className="w-full max-w-xl space-y-6">
          <label className="block text-gray-700 text-sm font-semibold mb-1">Jenis Tanaman</label>
          <select value={plantType} onChange={(e) => setPlantType(e.target.value)} className="w-full border-b-2 border-gray-300 focus:border-gray-400 outline-none text-gray-600 py-2">
          ...
          {isCameraOpen ? (
            <div className="flex flex-col items-center">
              <Webcam audio={false} ref={webcamRef} screenshotFormat="image/jpeg" className="w-full h-auto max-h-64 object-cover rounded-lg border border-gray-300" />
              <button className="mt-4 px-4 py-2  bg-[#6D7E5E] text-white rounded-lg" onClick={handleCapture}>
                Ambil Foto
              </button>
              <button className="mt-2 px-4 py-2 bg-gray-400 text-white rounded-lg" onClick={() => setIsCameraOpen(false)}>
                Batal
              </button>
            </div>
          ) : (
          ...
          <div className="flex space-x-4">
            ...
            <button className="flex items-center px-4 py-2 border border-gray-400 text-gray-700 rounded-lg hover:bg-green-100" onClick={() => setIsCameraOpen(true)}>
              <span className="text-xl mr-2">+</span>Potret
            </button>
          </div>
          <div className="flex justify-end mt-8">
            <button className={`w-full max-w-28  bg-[#6D7E5E] text-white font-semibold py-2 border border-green-700 rounded-lg ${loading ? 'opacity-50' : ''}`} onClick={handleDeteksiClick} disabled={loading}>
