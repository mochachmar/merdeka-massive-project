...
function AdminArtikel() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await axios.delete(`http://localhost:5000/api/articles/${selectedItem.article_id}`);
      console.log('Server response:', response.status); // Debugging

      if (response.status === 200 || response.status === 204) {
        // Remove the deleted article from the state
        setArticles((prevArticles) =>
          prevArticles.filter((article) => article.article_id !== selectedItem.article_id)
        );
        closeModal();
                  ...
                  {articles.map((item, index) => (
                    <tr key={item.article_id} className="border-b hover:bg-gray-100">
                      ...
                      <td className="px-4 py-2">
                        <img
                          src={`http://localhost:5000/uploads/${item.thumbnail_image || 'default-image.jpg'}`}
                          alt={item.title}
                      ...
                      <td className="px-4 py-2 text-center">
                        <span
                          className={`px-3 py-1 rounded-full text-white font-semibold ${item.status === 'published' ? 'bg-green-500' : 'bg-yellow-500'}`}
                        >
                      ...
                      <td className="px-4 py-2 text-center">
                        <button
                          onClick={() => handleEditArtikel(item.article_id)}
