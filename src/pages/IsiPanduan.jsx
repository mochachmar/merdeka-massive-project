                ...
                {data.map((item, index) => (
                  <tr key={item.id} className="border-b hover:bg-gray-100">
                    ...
                    <td className="px-4 py-2 text-center">
                      <span className={`px-3 py-1 rounded-full text-white font-semibold ${item.status === 'Diterbikan' ? 'bg-green-500' : 'bg-yellow-500'}`}>{item.status}</span>
                    </td>
                    <td className="px-4 py-2 text-center">
                      <button onClick={handleEditIsiPanduan} className="bg-yellow-500 text-white px-2 py-1 rounded mr-2 hover:bg-yellow-600">
                        ✏️
                      </button>
          ...
          {isModalOpen && (
            <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
              <div className="bg-white rounded-lg shadow-lg p-6 w-80 text-center relative">
                ...
                <h2 className="text-lg font-semibold mb-4">Apakah Anda yakin ingin menghapus isi Panduan ini?</h2>
